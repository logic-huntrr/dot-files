...
shot_to_clipboard() {
    ...
    # 2. Capture with grim and pipe (-) to stdout
    # 3. Copy to clipboard with wl-copy
    if grim -g "$(slurp)" - | wl-copy; then
        # Play success sound using your existing script path
        "${sDIR}/Sounds.sh" --screenshot
        notify-send -t 3000 -i "${iDIR}/picture.png" "Screenshot" "Region copied to clipboard"
    else
        "${sDIR}/Sounds.sh" --error
    fi
}
