...
echo "" | rofi -dmenu -config "$rofi_theme" -mesg "$msg" | while read -r query; do
case "$query" in
    http://*|https://*|*.*) brave --class=rofi-search-browser --user-data-dir=$HOME/.config/BraveSoftware/Rofi-Search --new-window "$query" ;;
    *) brave --class=rofi-search-browser --user-data-dir=$HOME/.config/BraveSoftware/Rofi-Search --new-window "https://search.brave.com/search?q=$query" ;;
  esac
done
